...
exports.triggerTimeoutCheck = exports.jobTimeoutMonitor = void 0;
 ...
 */
exports.jobTimeoutMonitor = functions.pubsub
 ...
 */
async function checkStuckAcceptedJobs() {
    const now = new Date();
 ...
 */
async function checkStuckStartedJobs() {
    const now = new Date();
...
async function createAdminAlert(alertData) {
    try {
        const notification = Object.assign(Object.assign({}, alertData), { createdAt: admin.firestore.Timestamp.now(), resolved: false, source: 'job_timeout_monitor' });
 ...
 */
exports.triggerTimeoutCheck = functions.https.onCall(async (data, context) => {
    functions.logger.info('🔧 Manual timeout check triggered');
    // Verify admin authorization
    if (!context.auth || !context.auth.token.admin) {
        throw new functions.https.HttpsError('permission-denied', 'Only admins can trigger manual timeout checks');
    }
    try {
        const results = {
...
//# sourceMappingURL=jobTimeoutMonitor.js.map