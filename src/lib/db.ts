  ...
  async createTask(taskData: Omit<Task, 'id' | 'created_at' | 'updated_at'>): Promise<DatabaseResponse<Task>> {
    ...
    if (newTask.assigned_to) {
      try {
        ...
        import('@/lib/notifications/notificationTriggers').then(({ NotificationTriggers }) => {
          NotificationTriggers.onTaskAssigned(newTask)
        }).catch(error => {
          console.error('Error sending task assignment notification:', error)
        })
      } catch (error) {
        console.error('Error importing notification triggers:', error)
      }
    }
  ...
  async updateTask(id: string, updates: Partial<Task>): Promise<DatabaseResponse<Task>> {
    ...
    if (previousStatus !== 'completed' && this.tasks[index].status === 'completed') {
      try {
        // Use dynamic import to avoid circular dependencies
        import('@/lib/notifications/notificationTriggers').then(({ NotificationTriggers }) => {
          NotificationTriggers.onTaskCompleted(this.tasks[index])
        }).catch(error => {
