...
import {
  ...
  Building2, Phone, Mail, FileText, Tag
} from 'lucide-react'
...
// Types
import { JobType, JobPriority } from '@/services/JobAssignmentService'
...
const steps = [
  ...
  { id: 3, title: 'Priority & Duration', description: 'Set priority and time estimate' },
...
const jobTypes: { value: JobType; label: string; icon: React.ReactNode; description: string }[] = [
  { value: 'cleaning', label: 'Cleaning', icon: <Sparkles className="w-6 h-6" />, description: 'Regular or deep cleaning tasks' },
  { value: 'maintenance', label: 'Maintenance', icon: <Wrench className="w-6 h-6" />, description: 'Repairs and maintenance work' },
  { value: 'inspection', label: 'Inspection', icon: <Eye className="w-6 h-6" />, description: 'Property inspection and assessment' },
  { value: 'setup', label: 'Setup', icon: <Settings className="w-6 h-6" />, description: 'Property setup and preparation' },
  { value: 'checkout', label: 'Checkout', icon: <CheckCircle className="w-6 h-6" />, description: 'Guest checkout and property check' },
  { value: 'emergency', label: 'Emergency', icon: <AlertCircle className="w-6 h-6" />, description: 'Urgent emergency response' }
]

const priorities: { value: JobPriority; label: string; color: string; description: string }[] = [
  { value: 'low', label: 'Low', color: 'text-green-400', description: 'Can be completed when convenient' },
  { value: 'medium', label: 'Medium', color: 'text-yellow-400', description: 'Should be completed within normal timeframe' },
  { value: 'high', label: 'High', color: 'text-orange-400', description: 'Needs prompt attention' },
  { value: 'urgent', label: 'Urgent', color: 'text-red-400', description: 'Requires immediate action' }
]
...
export function CreateJobWizardModal({
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.properties) {
          ...
          setProperties(data.properties.map((prop: any) => ({
            id: prop.id,
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.data) {
          ...
          setStaff(data.data.map((member: any) => ({
            id: member.id,
  ...
  const canProceed = () => {
    switch (currentStep) {
      case 0: return formData.jobType
      case 1: return formData.title && formData.description
      case 2: return formData.propertyId
      case 3: return formData.priority && formData.estimatedDuration > 0
      case 4: return formData.scheduledDate && formData.scheduledStartTime
      case 5: return formData.assignedStaffId
      case 6: return true // Requirements are optional
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          <h3 className="text-xl font-semibold text-white mb-2">Select Job Type</h3>
          <p className="text-gray-400">Choose the type of job you want to create</p>
        </div>
  ...
  function renderBasicInfoStep() {
    return (
      <div className="space-y-6">
        <div className="text-center">
          <h3 className="text-xl font-semibold text-white mb-2">Job Information</h3>
  ...
  function renderPropertyStep() {
    return (
      <div className="space-y-6">
        <div className="text-center">
          <h3 className="text-xl font-semibold text-white mb-2">Select Property</h3>
          <p className="text-gray-400">Choose the property where the job will be performed</p>
        </div>
  ...
  function renderPriorityStep() {
    return (
      <div className="space-y-6">
        <div className="text-center">
          <h3 className="text-xl font-semibold text-white mb-2">Priority & Duration</h3>
          <p className="text-gray-400">Set the job priority and estimated duration</p>
        </div>
          ...
          <div>
            ...
            <div className="flex items-center space-x-4">
              <Button
                ...
                onClick={() => setFormData({
                  ...formData,
                  estimatedDuration: Math.max(15, formData.estimatedDuration - 15)
                })}
              ...
              <Input
                ...
                onChange={(e) => setFormData({
                  ...formData,
                  estimatedDuration: Math.max(15, parseInt(e.target.value) || 15)
                })}
  ...
  function renderSchedulingStep() {
    return (
      <div className="space-y-6">
        <div className="text-center">
          <h3 className="text-xl font-semibold text-white mb-2">Schedule Job</h3>
  ...
  function renderStaffAssignmentStep() {
    return (
      <div className="space-y-6">
        <div className="text-center">
          <h3 className="text-xl font-semibold text-white mb-2">Assign Staff</h3>
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          ...
          <p className="text-gray-400">Specify any special requirements or instructions</p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => setFormData({
                ...formData,
                requiredSupplies: e.target.value.split(',').map(s => s.trim()).filter(s => s)
              })}
  ...
  function renderReviewStep() {
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          <h3 className="text-xl font-semibold text-white mb-2">Review Job Details</h3>
          <p className="text-gray-400">Please review all details before creating the job</p>
        </div>
        ...
        <div className="bg-gray-800/50 rounded-lg p-6 space-y-4">
            ...
            <div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-white">{formData.scheduledStartTime}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {formData.specialInstructions && (
            <div>
              <h4 className="font-semibold text-white mb-2">Special Instructions</h4>
              <p className="text-gray-300 text-sm">{formData.specialInstructions}</p>
            </div>
          )}
        </div>
      </div>
