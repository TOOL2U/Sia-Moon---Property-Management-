...
export async function middleware(request: NextRequest) {
  ...
  if (
    ...
    console.log(
      '🔓 MIDDLEWARE: Mobile API endpoint detected, allowing access immediately'
    )
    return NextResponse.next()
  }
  ...
  if (
    ...
    console.log(
      '🔓 MIDDLEWARE: Test/Debug API endpoint detected, allowing access'
    )
    return NextResponse.next()
  }
  ...
  if (isDevelopmentBypass || isDevelopmentSessionBypass) {
    ...
    if (isDevelopmentSessionBypass) {
      console.warn(
        '🔄 SESSION BYPASS ENABLED - DEVELOPMENT SSR SYNC WORKAROUND'
      )
    }
    return NextResponse.next()
  }
  ...
  console.log(
    '🔍 MIDDLEWARE: firebaseToken:',
    firebaseToken ? 'EXISTS' : 'MISSING'
  )

  // For now, let's be more strict and require a valid, non-empty token
  const hasValidToken =
    (authToken && authToken.length > 10) ||
    (firebaseToken && firebaseToken.length > 10)

  if (!hasValidToken) {
    console.log(
      '🔒 MIDDLEWARE: No valid authentication token found, redirecting to login'
    )
  ...
  console.log(
    '✅ MIDDLEWARE: Valid authentication token found, allowing access'
  )
  return NextResponse.next()
}
